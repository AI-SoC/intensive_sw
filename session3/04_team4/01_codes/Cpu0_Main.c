...
const unsigned int hyundai[6] = {207, 220, 329, 311, 370, 554};
typedef enum{
    RED = 0, 
    GREEN, 
    BLUE,
...
unsigned int red_timer = 0;
bool distance_valid_flag = false;
...
void initGTM(void){
    ...
    GTM_TOM0_TGC1_GLB_CTRL.U |= 0x2 << UPEN_CTRL3_BIT_LSB_IDX;      // TOM0 channel 11 on update trigger
    GTM_TOM0_TGC1_ENDIS_CTRL.U |= 0x2 << ENDIS_CTRL3_BIT_LSB_IDX;   // enable channel 11 on update trigger
    GTM_TOM0_TGC1_OUTEN_STAT.U |= 0x2 << OUTEN_CTRL3_BIT_LSB_IDX;   // enable channel 11 output on update trigger
    ...
    GTM_TOM0_CH11_SR0.U = 12500 -1;     //PMW freq. = 6250 kHz / 12500 = 500 Hz (temp)
...
/********************************************************************
 *                                                                   
 *      Versatile Analog to Digital Conversion(VADC) functions                      
 *                                                                 
 ********************************************************************/
...
void buzzer_toggle(){
    if(is_buzzer_on) buzzer_off();
    else buzzer_on();
}

void hyundai_jingle(){
    const int term = 10000000;
    GTM_TOM0_CH11_CTRL.U &= ~(0x1 << OSM_BIT_LSB_IDX);      // continuous mode
    GTM_TOM0_TGC1_GLB_CTRL.U |= 0x1 << HOST_TRIG_BIT_LSB_IDX;
    
    for(int i = 0 ; i < 3; i++){
        GTM_TOM0_CH11_SR0.U = (6250000 / hyundai[i]) - 1;     // duty cycle = 6250 / 12500 = 50% (temp)
        GTM_TOM0_CH11_SR1.U = (3125000 / hyundai[i]) - 1;     // duty cycle = 6250 / 12500 = 50% (temp)
    ...
    GTM_TOM0_CH11_CTRL.U &= ~(0x1 << OSM_BIT_LSB_IDX);      // continuous mode
    GTM_TOM0_TGC1_GLB_CTRL.U |= 0x1 << HOST_TRIG_BIT_LSB_IDX;
    for(unsigned int j = 0 ; j < term/5; j++);
    ...
    for(unsigned int i = 0 ; i < 2*term; i++);
    buzzer_off();
}
...
void set_rgb(led_color c){
    switch (c){
    case RED:
        P02_OUT.U |= 0x1 << P7_BIT_LSB_IDX;     // red light
        P10_OUT.U &= ~(0x1 << P5_BIT_LSB_IDX);
        P10_OUT.U &= ~(0x1 << P3_BIT_LSB_IDX);
        break;
    case GREEN:
        P02_OUT.U &= ~(0x1 << P7_BIT_LSB_IDX);
        P10_OUT.U |= 0x1 << P5_BIT_LSB_IDX;     // green light
        P10_OUT.U &= ~(0x1 << P3_BIT_LSB_IDX);
        break;
    case BLUE:
        P02_OUT.U &= ~(0x1 << P7_BIT_LSB_IDX);
        P10_OUT.U &= ~(0x1 << P5_BIT_LSB_IDX);
        P10_OUT.U |= 0x1 << P3_BIT_LSB_IDX;     // blue light
        break;
    case BLACK:
        P02_OUT.U &= ~(0x1 << P7_BIT_LSB_IDX);
        P10_OUT.U &= ~(0x1 << P5_BIT_LSB_IDX);
        ...
        break;
    case WHITE:
        P02_OUT.U |= 0x1 << P7_BIT_LSB_IDX;     // blue light
        P10_OUT.U |= 0x1 << P5_BIT_LSB_IDX;     // blue light
        P10_OUT.U |= 0x1 << P3_BIT_LSB_IDX;     // blue light
        break;
    default:
        break;
...
void set_timer_freq(int freq){
    CCU60_T12PR.U = freq - 1;    // PM interrupt freq. = f_T12 / (T12PR + 1)
...
void ERU0_ISR(void){
    ...
    else    // falling edge of echo
    {
        ...
        distance = ((CCU61_T12.B.T12CV * 1000000) / 48828) /58;
...
// Timer ISR to manipulate LEDs
__interrupt(0x0B) __vector_table(0)
void CCU60_T12_ISR(void){
    if(red_timer <= RED_DELAY && red_timer) red_timer--;
    P02_OUT.U &= ~(0x1 << P6_BIT_LSB_IDX);
    P10_OUT.U ^= 0x1 << P1_BIT_LSB_IDX;

    if(distance >= DIST_THRESHOLD && !red_timer){
        P10_OUT.U |= 0x1 << P2_BIT_LSB_IDX; // set to 1 P10.2 (LED D13 BLUE)
        buzzer_off();
    }
    else{
...
int core0_main(void)
{
    ...
    while(1)
    {
        ...
        usonicTrigger();
        while(!distance_valid_flag);
        VADC_startConversion();
        prev_adc_value = adc_value;
        adc_value = VADC_readResult();
        if(!(~prev_adc_value)) prev_adc_value = adc_value;

        // blue     --> (large distance) and (zero red_timer)
        if(distance >= 60 && !red_timer){
            set_rgb(BLUE);
            set_timer_freq(24000);
        }
        // green    --> (small delta) and (zero red_timer) and (small distance)
        if(abs(prev_adc_value - adc_value) < 400 && !red_timer && distance < DIST_THRESHOLD){
            set_rgb(GREEN);
            // P10_OUT.B.P1 = 0x0;
            set_timer_freq(12000);
        }
        // red      --> (positive red_timer) or ((large delta) and (small distance))
        if(red_timer || (abs(prev_adc_value - adc_value) >= 400 && distance < DIST_THRESHOLD)){
             // red_timer || distance < DIST_THRESHOLD || delta >= 400
            if(!red_timer) red_timer = RED_DELAY;  
            set_rgb(RED);
            // P10_OUT.B.P1 = 0x1;
            set_timer_freq(4000);
        }
