...
void initGTM(void)
{
    ...
    while((GTM_CLC.U & (1 << DISS_BIT_LSB_IDX)) != 0);      // wait until GTM module enabled (체크)

    // GTM clock configuration
    GTM_CMU_FXCLK_CTRL.U &= ~(0xF << FXCLK_SEL_BIT_LSB_IDX);        // input clock of CMU FXCLK --> CMU_GCLK_EN     1. 클락 선택부터
    GTM_CMU_CLK_EN.U |= 0x2 << EN_FXCLK_BIT_LSB_IDX;                // enable all CMU_FXCLK                         2. 이후 enable (글리치 발생 방지)
    ...
    GTM_TOM0_CH1_CTRL.U |= 0x1 << CLK_SRC_CR_BIT_LSB_IDX;           // clock source --> CMU_FXCLK(1) = 6250 kHz
...
unsigned int VADC_readResult(void)
{
    ...
    result = VADC_G4_RES0.U & (0xFFF << RESULT_BIT_LSB_IDX);    // read ADC value           --> 하위 12비트만 가져오겠다
...
void initCCU60(void)
{
    ...
    CCU60_T12PR.U =  30000 -1;                      // PM interrupt freq. = f T12 / (T12PR + 1) (24414 : 0.5s)
...
int core0_main(void)
{
    ...
    while(1)
    {
        ...
        if( adcResult >= 3096)
        {
            ...
            //GTM_TOM0_CH1_SR1.U = 0;                                 // LED RED 밝기 조절
        }
        else if( adcResult >= 2048 )
        {
            ...
            ///GTM_TOM0_CH1_SR1.U = 1000;                             // LED RED 밝기 조절
        }
        else if( adcResult >= 1024 )
        {
            ...
            //GTM_TOM0_CH1_SR1.U = 7000;                             // LED RED 밝기 조절
        }
        else
        {
            P02_OUT.U |= 0x1 << P7_BIT_LSB_IDX;
            ...
            //GTM_TOM0_CH1_SR1.U = 12500;                            // LED RED 밝기 조절
        }
    }
    return (1);
}
