...
int core0_main(void)
{
    ...
    GTM_TOM0_TGC0_GLB_CTRL.U |= 0x1 << HOST_TRIG_BIT_LSB_IDX; // first update trigger
    ...
    while(1)
    {
        ...
        for (unsigned int i = 0; i < 100; i++); // delay

        // Lab. continuous LED dimming by ADC
        ...
        if (adcResult >= 3096) {
            ...
            //GTM_TOM0_CH1_SR1.U = 0; // PWM duty cycle
        }
            ...
            //GTM_TOM0_CH1_SR1.U = 1000; // PWM duty cycle
        }
            ...
            //GTM_TOM0_CH1_SR1.U = 7000; // PWM duty cycle
        }
        else {
            // white
            P02_OUT.U |= 0x1 << P7_BIT_LSB_IDX;
            ...
            //GTM_TOM0_CH1_SR1.U = 12500; // PWM duty cycle
        }
    }
    return (1);
}
...
void initGTM() {
    clearENDINIT();
    // Generic Timer Module, Clock Control Register
    ...
    // CLock Management Unit, Fixed Clock Generation Unit
    GTM_CMU_FXCLK_CTRL.U &= ~(0xF << FXCLK_SEL_BIT_LSB_IDX); // input clock of CMU FXCLK --> fastest(100 Mhz)
    ...
    // before update, write on the shadow register

    GTM_TOM0_TGC0_ENDIS_CTRL.U |= 0x2 << ENDIS_CTRL1_BIT_LSB_IDX; // enable channel 1 on an update trigger

    GTM_TOM0_TGC0_OUTEN_CTRL.U |= 0x2 << OUTEN_CTRL1_BIT_LSB_IDX; // enable channel 1 output on an update trigger 

    // TOM0 Channel x Control Register
    GTM_TOM0_CH1_CTRL.U |= 0x1 << SL_BIT_LSB_IDX; // high signal level for duty cycle
    GTM_TOM0_CH1_CTRL.U |= 0x1 << CLK_SRC_SR_BIT_LSB_IDX; // clock source
    // CMU_FXCLK(1) --> FXCLKSEL / 2^4 --> 100 Mhz / 16 = 6250 Khz 
    GTM_TOM0_CH1_CTRL.U &= ~(0x1 << OSM_BIT_LSB_IDX); // continuous mode enable
    GTM_TOM0_CH1_CTRL.U &= ~(0x1 << TRIOUT_BIT_LSB_IDX); // TRIG[x] = TRIG[x-1]

    // want to make period 2 ms
    GTM_TOM0_CH1_SR0.U = 12500 - 1; // 6250 * 2 - 1

    GTM_TOM0_CH1_SR1.U = 1250 - 1; // duty cycle = 1250 / 12500 = 10 % (temporary)

    GTM_TOUTSEL6.U &= ~(0x3 << SEL7_BIT_LSB_IDX); // GTM output --> TOUT103 --> TOM0_1
    // TOUTSEL6(Timer Output Select 6 Register) controls TOUT96 ~ 111
    // 103 = 16 * 6 + 7 -> SEL7, 00 --> Timer A
}
