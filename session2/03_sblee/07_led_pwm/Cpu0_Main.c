...
void initGTM(void)
{
    ...
    GTM_CMU_FXCLK_CTRL.U &= ~(0xF << FXCLK_SEL_BIT_LSB_IDX);        // input clock of CMU FXCLK --> 순서가 중요! 값을 바꾸고 enable해야됨! 안 그러면 글리치가 발생!
    ...
    GTM_TOM0_TGC0_OUTEN_CTRL.U |= 0x2 << OUTEN_CTRL1_BIT_LSB_IDX;   // enable chaanel 1 output on update trigger
...
void initERU(void)
{
    ...
    SCU_IGCR0.U &= ~(0x3    << IGP0_BIT_LSB_IDX); // OGU0 By Pass trigger
...
void initCCU60(void)
{
    ...
    CCU60_T12PR.U = 24414 -1;                       // PM interrupt freq. = f_T12 / (T12PR + 1) , 48828 / 2 -1 => 0.5s
    ...
    SRC_CCU6_CCU60_SR0.U |= 0x1 << SRE_BIT_LSB_IDX;         // T12 start counting


    // CCU60 T12 counting start
    CCU60_TCTR4.U = 0x1 << T12RS_BIT_LSB_IDX;       // T12 startcounting
}
...
void initVADC(void)
{
    ...
    VADC_G4_QMR0.U |= 0x1 << ENGT_BIT_LSB_IDX;          // enable conversion request // not use request gating
...
int core0_main(void)
{
    ...
    GTM_TOM0_TGC0_GLB_CTRL.U |= 0x1 << HOST_TRIG_BIT_LSB_IDX;       // trigger update request signal --> 최초 한번 SR0으로부터 CM0로 카피
