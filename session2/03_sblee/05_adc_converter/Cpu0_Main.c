...
void initVADC(void)
{
    ...
    VADC_G4_QMR0.U |= 0x1 << ENGT_BIT_LSB_IDX;          // not use request gating
...
int core0_main(void)
{
    ...
    while(1)
    {
        ...
        P10_OUT.U ^= 0x1 << P1_BIT_LSB_IDX;             // toggle P10.1 (LEX D12 RED)

        for(uint32 i=0;i<10000000;i++);     // software delay

        P10_OUT.U ^= 0x1 << P2_BIT_LSB_IDX;             // toggle P10.2 (LEX D13 RED)
        */
        ...
        {
            P10_OUT.U |= 0x1 << P1_BIT_LSB_IDX;         //(LED D13 RED)
        }
        else
        {
            P10_OUT.U &= ~(0x1 << P1_BIT_LSB_IDX);      // clear P10.1 (LED D13 RED)
