...
void initGTM(void){
    ...
    GTM_TOM0_TGC0_OUTEN_CTRL.U |= 0x2 << OUTEN_CTRL1_BIT_LSB_IDX;   // enable channel 1 on update trigger
    ...
    GTM_TOM0_CH1_SR0.U = 12500 - 1;                                  // Period set : PWM freq. = 6250 kHz/ 12500 = 500 Hz
    GTM_TOM0_CH1_SR1.U = 1250 - 1;                                   // duty set : 1250 / 12500 = 10%
...
void initPWM_LED(void){
    ...
    P10_IOCR0.U |= 0x11 << PC1_BIT_LSB_IDX;     // set to p10.1 GTM output (PWM)
    P10_IOCR0.U |= 0x10 << PC2_BIT_LSB_IDX;     // set to p10.2 push-pull general output
}
...
// ### if button is pressed, LED is on much than faster
__interrupt(0x0A) __vector_table(0)
void ERU0_ISR(void){
    // button status check
    if((P02_IN.U & (0x1 << P1_BIT_LSB_IDX)) == 0){  // button pushed
        P10_OUT.B.P1 = 0x1;
        CCU60_T12PR.U = 12207 - 1;                  // PM interrupt freq.
        CCU60_TCTR4.U |= 0x1 << T12STR_BIT_LSB_IDX; // load T12PR from shadow register
    }
    else{                                           // button released
        P10_OUT.B.P1 = 0x0;
        CCU60_T12PR.U = 24414 - 1;                  // PM interrupt freq.
        CCU60_TCTR4.U |= 0x1 << T12STR_BIT_LSB_IDX; // load T12PR from shadow register
    }
}
...
void CCU60_T12_ISR(void){
    // ### LED blue is toggled every time
...
//    initButton();
    ...
    GTM_TOM0_TGC0_GLB_CTRL.U |= 0x1 << HOST_TRIG_BIT_LSB_IDX; // initiate trigger request signal before start

    while(1)
    {
        // ### Polling-based VADC conversion
        // Get VADC data -> Show certain colors w.r.t. certain VADC data value
...
//            GTM_TOM0_CH1_SR1.U = 12500;
        ...
        unsigned short duty = 12500 * adcResult / 4096;
