...
void initGTM()
{
    // Password access to unlock SCU_WDTSCON0
       ...
       GTM_TOM0_CH1_CTRL.U |= 0X1 <<CLK_SRC_SR_BIT_LSB_IDX;     // clock source --> CMU FXCLK(!) = 6250 kHz
       ...
       GTM_TOUTSEL6.U &= ~(0X3 << SEL7_BIT_LSB_IDX);         // TOUT103 --> COM0 channel 1
                                                               // 103 = 16 * 6 + 7
...
void initRGBLED(void)
{
    ...
    P02_IOCR4.U &= ~(0x1f << PC7_BIT_LSB_IDX); //D9 PORT 2.7
    P10_IOCR4.U &= ~(0x1f << PC5_BIT_LSB_IDX); //D10 PORT 10.5
    P10_IOCR0.U &= ~(0X1F << PC3_BIT_LSB_IDX); //D11 PORT 10.3
...
void initVADC(void)
{
    ...
    VADC_G4_QMR0.U &= ~(0X3 << ENGT_BIT_LSB_IDX);   // doesn't use gating mode
    VADC_G4_QMR0.U |= 0X1 << ENGT_BIT_LSB_IDX;  // enable conversoin request
